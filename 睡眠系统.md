...
- `purpose`: 用途，可选，默认为"login"，可选值：login、setpwd等
...
# 用户未完善信息
{
    ...
    'datainfo': {'id': 9, 
                 ...
                 'gender': '1', 
                 'birthday': None, 
                 'created_at': '2026-01-28T16:32:41.483540+08:00', 
                 'updated_at': '2026-01-28T17:46:05.369901+08:00', 
                 'session_id': 'cac885643c194eeba56cef45786b8231', 
                 'wechat_openid': None, 
                 'wechat_unionid': None, 
...
**接口地址:** `POST /api/v1/user/unsubscribe`
...
```json
{
    ...
    "datainfo": {
	 "fall_sleep":"10:50", # 入睡时间
 	 "sleep_end":"10:50", # 睡眠结束时间
     'sleep_end_dt':"12387362", # 结束助眠时间（毫秒）
     'sleep_start':"12387362", # 开始时间（毫秒）
     'is_latest': True, # 是否有数据还在生成报告
     'sleepiness_value' :12.3 # 睡意值 
     "awake_minutes":24, # 清醒时长
     "wake_times_str":"10:50", # 醒来时间
    "deep_sleep_minutes":0,   # 深度睡眠时长
    "light_sleep_minutes":0, # 浅睡时长
	...
	'rem_minutes_diff': 0，
	'report_date': '2025-11-26'
	'sleep_status':"stop", # 自动停止时返回stop, 其余时间返回running
}  
...
```json
{
    ...
    "datainfo": {'fall_sleep': '17:27', 
                 'sleep_end': '15:40', 
                 'sleep_end_dt':"12387362",
                 'sleep_start':"12387362",
                 'awake_minutes': 27, 
                 ...
                 'all_during': 27, 
                 'rem_sleep_minutes': 0, 
                 'sleep_stage': {'date': '17:27', 'predict_result': ['Waking']}, {'date': '17:27', 'predict_result': ['NO']}],   # 状态NO为空补偿
				...
				'sleep_brain_wave':{"data":[1.7,1.6],"start_time":'1773645020832',"end_time":'1773647708200'}, # 脑电波
				'sleep_posture': [1,2,3,9] # 睡眠姿势  状态 9 为空补偿
				'sleep_analysis':
{'deep_eval': 0, 'deep_status': -1, 'n1_eval': 0, 'n1_status': -1, 'n2_eval': 0, 'n2_status': -1, 'rem_eval': 0, 'rem_status': -1, 'awake_eval': 0, '0': -1} # 1：偏多，0：正常，-1：不足, 数组第一个百分比，第二个状态	
				'sleepiness_value': 2.1， # 睡意值
				'turn_over':1，# 翻身次数
				'asleep_time': 2， # 入睡时长
				'sleepiness_level':{'dense_asleep': 0, 'half_asleep': 10, 'obvious_asleep': 1，"half_asleep_percent": ,
                    "obvious_asleep_percent": ,
                    "dense_asleep_percent": 
...
**接口地址:** `POST /api/v1/sleep_algorithm/sleep_info_start`
...
**接口地址:** `POST /api/v1/sleep_algorithm/brain_music_list`
...
**接口地址:** `POST /api/v1/sleep_algorithm/brain_music_recommend`
